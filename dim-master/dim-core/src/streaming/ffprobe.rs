...
pub struct Stream {
    #[serde(default)]
    pub index: i64,
    /// NOTE: attachment/data streams (e.g. font attachments with unknown
    /// mimetypes in fansub MKVs) come back from ffprobe WITHOUT a codec_name.
    /// This must tolerate that — one unparsable stream used to silently wipe
    /// the entire probe result and make the file unplayable.
    #[serde(default)]
...
#[derive(Default, Debug, Clone, PartialEq, Serialize, Deserialize)]
// ffprobe omits any field it can't compute (bit_rate, start_time, ...) —
// every field must tolerate absence or one odd container breaks the parse.
#[serde(default)]
struct Format {
    pub filename: String,
    pub nb_streams: i64,
...
impl FFProbeCtx {
    ...
    pub async fn get_meta(&self, file: impl ToString) -> Result<FFPStream, std::io::Error> {
        ...
        Ok(parse_probe_output(&json))
    }
}

/// Parse ffprobe JSON leniently, one stream at a time: real-world files carry
/// attachment/data streams with shapes we can't anticipate, and one
/// unparsable stream must never wipe the whole probe result (that
/// made entire files unplayable).
pub(crate) fn parse_probe_output(json: &str) -> FFPStream {
    match serde_json::from_str::<serde_json::Value>(json) {
        Ok(value) => {
            let streams = value
                ...
                .map(|arr| {
                    arr.iter()
                        .filter_map(|raw| {
                            match serde_json::from_value::<Stream>(raw.clone()) {
                                Ok(stream) => Some(stream),
                                Err(error) => {
                                    error!(
                                        %error,
                                        stream = %raw,
                                        "Skipping unparsable ffprobe stream."
                                    );
                                    None
                                }
                            }
            ...
            let format = value
                ...
                .cloned()
                .and_then(|f| {
                    serde_json::from_value::<Format>(f)
                        .map_err(|error| {
                            error!(%error, "Failed to parse ffprobe format section.")
                        })
                        .ok()
                })
                .unwrap_or_default();

            // Only corrupt if we truly got nothing usable.
            let corrupt = streams.is_empty();
            FFPStream {
                streams,
                format,
                corrupt,
        ...
        Err(error) => {
            error!(
                %error,
                json_head = &json[..json.len().min(512)],
                "ffprobe emitted invalid JSON; treating file as corrupt."
            );
            FFPStream::default()
        }
    }
...
mod tests {
    ...
    /// Fansub MKVs attach fonts with unknown mimetypes; ffprobe reports those
    /// attachment streams WITHOUT a codec_name. The probe parse must survive
    /// them — this exact shape made real files completely unplayable.
    #[test]
    fn tolerates_attachment_without_codec_name() {
        let json = r#"{
            "streams": [
                ...
                {"index": 4, "codec_type": "data", "codec_name": 42, "width": "not-a-number"}
            ],
            "format": {
                "filename": "x.mkv",
                "nb_streams": 4,
                "format_name": "matroska,webm",
                "duration": "1420.032000"
            }
        }"#;

        let probe = parse_probe_output(json);
        assert!(!probe.corrupt);
        assert_eq!(probe.get_primary("video").unwrap().codec_name, "h264");
        assert_eq!(probe.get_primary("audio").unwrap().codec_name, "aac");
        assert_eq!(probe.get_duration(), Some(1420));
    }
}
