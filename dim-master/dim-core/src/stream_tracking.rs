...
impl VirtualManifest {
    ...
    pub fn set_transcode_reason(mut self, reason: Option<String>) -> Self {
        self.transcode_reason = reason;
        self
    }
...
impl StreamTracking {
    ...
    pub async fn compile_only(
        ...
        let included: Vec<&VirtualManifest> = manifests
            .iter()
            .filter(|x| filter.iter().any(|id| id == &x.id))
            .collect();

        // An includes list that matches nothing would produce an unplayable
        // empty MPD — fall back to the full manifest instead.
        if included.is_empty() {
            drop(lock);
            return self.compile(gid, start_num).await;
        }

        let duration = ts_to_xml(included.first().and_then(|x| x.duration)? as u64);
