...
impl MediafileCreator {
    ...
    pub async fn insert_batch<'a>(
        ...
        for mediafile in batch {
            ...
            // One failed insert must not abort (and roll back) the whole
            // batch of up-to-256 files — skip and keep indexing.
            let id = match mediafile
                .insert(&mut tx)
                ...
                .await
            {
                Ok(id) => id,
                Err(error) => {
                    warn!(?error, ?mediafile, "Failed to insert mediafile, skipping.");
                    continue;
                }
            };
