    ...
    if let Ok(conn) = dim_database::get_conn_logged().await {
        if let Ok(mut db_tx) = conn.read().begin().await {
            ...
            for lib in libs.drain(..) {
                ...
                let provider = crate::providers::provider_for(&lib.provider, media_type);
                ...
                // Scan first, THEN arm the watcher — running them
                // concurrently made both index the same files.
                tokio::spawn(async move {
                    ...
                    if let Err(error) = watcher.start_daemon().await {
                        tracing::error!(?error, library_id, "fs-watcher died");
                    }
                });
            }
        }
