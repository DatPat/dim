...
// NOTE: these MUST be `static`, not `const` — a `const` Lazy is instantiated
// anew at every use site, so every access rebuilt the provider (HTTP client,
// cache and all) from scratch.
pub static MOVIES_PROVIDER: Lazy<Arc<dyn ExternalQueryIntoShow>> = Lazy::new(|| {
    dim_core::providers::provider_for("tmdb", dim_database::library::MediaType::Movie)
});
pub static TV_PROVIDER: Lazy<Arc<dyn ExternalQueryIntoShow>> =
    Lazy::new(|| dim_core::providers::provider_for("tmdb", dim_database::library::MediaType::Tv));
...
pub async fn get_media_by_id(
    ...
    let progress = match media.media_type {
        MediaType::Episode | MediaType::Movie => {
            let (delta, duration) = Progress::get_progress_for_media(&mut tx, id, user.id)
                .await
                .unwrap_or((0, 1));
            // Watched items present as unstarted so clients offer a fresh
            // playthrough instead of resuming into the outro.
            let delta = if Progress::is_watched(delta, duration) {
                0
            } else {
                delta
            };
            Some(json!({ "progress": delta }))
        }
        MediaType::Tv => {
            if let Ok(Some(ep)) = Episode::get_last_watched_episode(&mut tx, id, user.id).await {
                ...
                if Progress::is_watched(delta, duration) && next_episode.is_ok() {
                    let next_episode = next_episode.unwrap();
                    let (delta, duration) =
                        Progress::get_progress_for_media(&mut tx, next_episode.id, user.id)
                            .await
                            .unwrap_or((0, 1));
                    let delta = if Progress::is_watched(delta, duration) {
                        0
                    ...
                    // Either mid-episode (resume) or the finale was watched
                    // to the outro (offer a replay, not a resume).
                    let delta = if Progress::is_watched(delta, duration) {
                        0
                    } else {
                        delta
    ...
    let next_episode_id = match Episode::get_by_id(&mut tx, id).await {
        Ok(ep) => {
            // When end-of-episode UI (next episode / return to show) should
            // surface: the last 10% of the episode being played. Based on
            // the CURRENT episode's duration — the next episode's runtime
            // says nothing about when this one ends.
            let credits_at =
                ep.media.get_first_duration(&mut tx).await as f64 * EPISODE_DONE_THRESH;
            ...
            // Lets clients link back to the show (e.g. on a series finale
            // where there is no next episode to offer).
            let tv_show_id = Season::get_tvshowid(&mut tx, ep.seasonid)
                .await
                .map(|id| json!({"tv_show_id": id}))
                .ok();

            Some(json!({
                "chapters": { "credits": credits_at },
                ..?next_episode,
                ..?prev_episode,
                ..?tv_show_id,
            }))
...
pub async fn tmdb_search(Query(params): Query<TmdbSearchParams>) -> Result<Response, Error> {
    ...
    let results = provider
        .search(&params.query, params.year, None)
        .await
