...
pub async fn get_image(
    ...
    // axum/matchit captures wildcard params with a leading `/` — strip it so
    // PathBuf::push appends instead of replacing the base path.
    ...
    */
    ...
    if !path::Path::new(&file_path).exists() {
        if let Ok(x) = asset::Asset::get_url_by_file(&mut tx, &url_path).await {
            insert_into_queue(x, path.into(), true).await;
        }
