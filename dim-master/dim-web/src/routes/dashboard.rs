...
async fn banner_for_movie(
    ...
    // Quitting during the credits counts as watched — don't advertise the
    // last few minutes as something left to continue.
    let progress = if Progress::is_watched(progress, media_duration) {
        0
    } else {
        progress
    };
...
async fn banner_for_show(
        ...
        Episode::get_last_watched_episode(&mut *conn, media.id, user.id).await
    {
        ...
        if Progress::is_watched(delta, duration) {
    ...
    // Reached when the finale was watched to the outro and there's no next
    // episode to advance to — present it as watched, not "in progress".
    let progress = if Progress::is_watched(progress, duration) {
        0
    } else {
        progress
    };
