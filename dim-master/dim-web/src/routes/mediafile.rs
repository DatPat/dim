...
pub async fn rematch_mediafile(
    ...
    let first_file = mediafiles.first().cloned();
    ...
    // For TV libraries, try to auto-match sibling files in the same directory.
    if media_type == MediaType::Tv {
        if let Some(matched_file) = first_file {
            let tmdb_id = route_args.tmdb_id.clone();
            let conn_clone = conn.clone();
            let provider_clone = provider.clone();
            tokio::spawn(async move {
                dim_core::scanner::smart_scan::match_siblings(
                    &conn_clone,
                    &matched_file,
                    &tmdb_id,
                    provider_clone,
