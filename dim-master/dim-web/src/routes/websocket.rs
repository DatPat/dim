...
pub async fn handle_websocket_session(
    ...
    watch_together: dim_core::watch_together::WatchTogetherManager,
) {
    ...
    let mut authed_user_id: Option<i64> = None;
    let mut authed_username: Option<String> = None;
            ...
            if let Ok(ClientActions::Authenticate { token }) = serde_json::from_str(&st) {
                if let Ok(token_data) = dim_database::user::Login::verify_cookie(token) {
                    if let Ok(mut sql_tx) = conn.read().begin().await {
                        ...
                        {
                            authed_user_id = Some(u.id.0);
                            authed_username = Some(u.username.clone());

                            // Register addr for Watch Together broadcasts
                            watch_together.register_user_addr(u.id.0, addr).await;
    ...
    let user_id = match authed_user_id {
        Some(id) => id,
        None => return,
    };
    let username = authed_username.unwrap_or_default();
    ...
    while let Some(message) = stream.next().await {
        ...
        let Ok(action) = serde_json::from_str::<ClientActions>(&text) else {
            continue;
        };
