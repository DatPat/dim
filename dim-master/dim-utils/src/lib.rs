...
/// Resolve a subtitle-filename language tag to `(iso639-2/B code, English
/// name)`. Sidecar files tag languages loosely — 2-letter ISO-639-1 ("en"),
/// 3-letter ISO-639-2 B or T ("ger"/"deu"), or a spelled-out name
/// ("english") — so match all of them, case-insensitively.
pub fn lang_from_subtitle_tag(tag: &str) -> Option<(&'static str, &'static str)> {
    if tag.is_empty() {
        return None;
    }
    dia_i18n::iso_639::LANG_CODES
        .iter()
        .find(|x| {
            (!x.v1().is_empty() && x.v1().eq_ignore_ascii_case(tag))
                || x.v2b().eq_ignore_ascii_case(tag)
                || (!x.v2t().is_empty() && x.v2t().eq_ignore_ascii_case(tag))
                || x.name().eq_ignore_ascii_case(tag)
        })
        .map(|x| (x.v2b(), x.name()))
}
