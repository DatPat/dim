...
export const automatchLibrary = (id) => async (dispatch, getState) => {
  const token = getState().auth.token;

  try {
    await fetch(`/api/v1/library/${id}/automatch`, {
      method: "POST",
      headers: { authorization: token },
    });
  } catch (err) {
    // automatch request failed silently
  }
};

export const scanLibrary = (id) => async (dispatch, getState) => {
  const token = getState().auth.token;

  try {
    await fetch(`/api/v1/library/${id}/scan`, {
      method: "POST",
      headers: { authorization: token },
    });
  } catch (err) {
    // scan request failed silently
  }
};
