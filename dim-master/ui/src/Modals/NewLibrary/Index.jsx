...
function NewLibraryModal(props) {
  ...
  }, [close, dispatch, mediaType, name, selectedFolders, provider]);
    ...
    <div className="modalBoxContainer">
      ...
      >
        <div className="modalNewLibrary">
          ...
          />
          <div className="providerSelection" style={{ margin: "10px 0" }}>
            <p style={{ marginBottom: 6 }}>Metadata provider</p>
            <div style={{ display: "flex", gap: 8 }}>
              {[
                { value: "tmdb", label: "TMDB" },
                { value: "tvmaze", label: "TVmaze", tvOnly: true },
                { value: "anilist", label: "AniList (Anime)" },
              ]
                .filter((p) => !p.tvOnly || mediaType === "tv")
                .map((p) => (
                  <button
                    key={p.value}
