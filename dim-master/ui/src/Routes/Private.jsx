...
import { checkAdminExists, updateAuthToken, logout } from "../actions/auth.js";
...
function PrivateRoute(props) {
  ...
  */
  ...
  // Clear stale auth when fetchUser fails (e.g. after DB reset, expired token).
  // Without this, a stale cookie keeps token set, blocking the redirect to login/register.
  useEffect(() => {
    if (user.fetched && user.error && (token || tokenInCookie)) {
      document.cookie = "token=;expires=Thu, 01 Jan 1970 00:00:00 GMT";
      dispatch(logout());
    }
  }, [user.fetched, user.error, token, tokenInCookie, dispatch]);
