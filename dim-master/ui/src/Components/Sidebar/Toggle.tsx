...
function Toggle(props: Props) {
  ...
  const versionFull = useSelector((store: RootState) => {
    const { data } = store.settings.globalSettings;
    return data.version || "";
  });

  // e.g. "aarch64 release | 2026-03-23 07:27 UTC | cpu: neon"
  const parts = versionFull.split(" | ");
  const versionShort = parts[1] || parts[0]; // show build date/time
  ...
  */
  ...
  return (
    <section className="sidebarToggleWrapper">
      <DimLogo />
      <span className="version" title={versionFull}>{versionShort}</span>
