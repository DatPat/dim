...
import { automatchLibrary } from "actions/library";
...
const MatchMedia = ({ data, refetch, mediafileSearch }: MatchMediaProps) => {
  ...
  const scanning = useSelector((store: any) => store.library.scanning);
  const isScanning = scanning.includes(parseInt(libraryId));

  const handleAutomatch = useCallback(() => {
    if (isScanning) return;
    dispatch(automatchLibrary(libraryId) as any);
  }, [dispatch, libraryId, isScanning]);
        ...
        <div className="match-left">
          <div className="match-head-row">
            ...
            {user.info.roles?.includes("owner") && (
              <button
                className={`automatchBtn${isScanning ? " scanning" : ""}`}
                onClick={handleAutomatch}
                disabled={isScanning}
                title={isScanning ? "Matching..." : "Auto-match all"}
              >
                <RefreshIcon />
                <span>{isScanning ? "Matching..." : "Auto-match"}</span>
              </button>
            )}
          </div>
          {isOpened && (
