...
fn main() {
    ...
    let async_main = async move {
        ...
        // Construct the unified sync engine
        let sync_engine = dim::sync_engine::SyncEngine::default();

        // Optionally start the Syncplay TCP server
        if global_settings.syncplay_enabled {
            let sp_addr = std::net::SocketAddr::new(
                std::net::IpAddr::V4(std::net::Ipv4Addr::new(0, 0, 0, 0)),
                global_settings.syncplay_port,
            );
            let sp_engine = sync_engine.clone();
            let sp_motd = global_settings.syncplay_motd.clone();
            let sp_password = global_settings.syncplay_password.clone();
            tokio::spawn(async move {
                dim::syncplay_server::run_syncplay_server(
                    sp_addr, sp_engine, sp_motd, sp_password,
                )
                .await;
            });
        }
