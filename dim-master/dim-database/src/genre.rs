...
impl InsertableGenre {
    ...
    pub async fn insert(&self, conn: &mut crate::Transaction<'_>) -> Result<i64, DatabaseError> {
        // Exact case-insensitive match. The old `UPPER(name) LIKE ?` treated
        // the genre name as a LIKE pattern and could never use an index.
...
impl InsertableGenreMedia {
    ...
    pub async fn insert_pair(
        ...
        // Atomic upsert — the old code's existence check was a join with no
        // join condition (a cartesian product), which could skip inserts or
        // return an unrelated genre id.
        let id = sqlx::query!(
            "INSERT INTO genre_media (genre_id, media_id)
            VALUES ($1, $2)
            ON CONFLICT(genre_id, media_id) DO NOTHING",
            genre_id,
            media_id
        )
        .execute(&mut *conn)
