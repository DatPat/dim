...
pub struct Library {
    ...
    /// Is library hidden?
    pub hidden: bool,
    /// Metadata provider used to match this library's content
    /// ("tmdb" | "tvmaze" | "anilist").
...
impl Library {
    ...
    pub async fn get_all(conn: &mut crate::Transaction<'_>) -> Vec<Self> {
        sqlx::query!(r#"SELECT id, name, media_type as "media_type: MediaType", hidden as "hidden: bool", provider FROM library WHERE NOT hidden"#)
            .fetch_all(&mut *conn)
            .await
            .unwrap_or_default()
            .into_iter()
    ...
    pub async fn get_one(
        ...
        let library = sqlx::query!(
            r#"SELECT id, name, media_type as "media_type: MediaType", hidden as "hidden: bool", provider FROM library
            WHERE id = ?"#,
            lib_id
        )
        .fetch_one(&mut *conn)
...
pub struct InsertableLibrary {
    ...
    pub locations: Vec<String>,
    pub media_type: MediaType,
    /// Metadata provider ("tmdb" | "tvmaze" | "anilist"); defaults to tmdb.
    #[serde(default = "default_provider")]
    pub provider: String,
}
...
impl InsertableLibrary {
    ...
    pub async fn insert(&self, conn: &mut crate::Transaction<'_>) -> Result<i64, DatabaseError> {
        let lib_id = sqlx::query!(
            r#"INSERT INTO library (name, media_type, provider) VALUES ($1, $2, $3)"#,
            self.name,
            self.media_type,
            self.provider
        )
