...
impl MediaFile {
    ...
    /// Find mediafile ids whose target_file matches a SQL LIKE pattern.
    /// Used by the Sonarr/Radarr webhook to resolve files that external
    /// services report under a different mount prefix than the one Dim
    /// indexes (e.g. `%/Show Name/Season 1/file.mkv`).
    pub async fn get_ids_by_file_pattern(
        conn: &mut crate::Transaction<'_>,
        pattern: &str,
    ) -> Result<Vec<i64>, DatabaseError> {
        Ok(
            sqlx::query_scalar("SELECT id FROM mediafile WHERE target_file LIKE ?")
                .bind(pattern)
                .fetch_all(&mut *conn)
                .await?,
        )
...
/// Lightweight record for building the directory-to-show mapping during smart scan.
#[derive(Debug)]
pub struct MediafileShowMapping {
    pub target_file: String,
    pub show_id: i64,
...
impl MediaFile {
    /// Returns (target_file, show_id, show_name) for all matched TV mediafiles in a library.
    /// Used by smart scan to build a directory→show mapping.
    pub async fn get_show_mapping_for_lib(
        conn: &mut crate::Transaction<'_>,
        library_id: i64,
    ) -> Result<Vec<MediafileShowMapping>, DatabaseError> {
        Ok(sqlx::query_as!(
            MediafileShowMapping,
            r#"SELECT
                mediafile.target_file AS "target_file!",
                parent_media.id AS "show_id!",
                parent_media.name AS "show_name!"
            FROM mediafile
            INNER JOIN episode ON mediafile.media_id = episode.id
            INNER JOIN _tblseason ON episode.seasonid = _tblseason.id
            INNER JOIN _tblmedia AS parent_media ON _tblseason.tvshowid = parent_media.id
            WHERE mediafile.library_id = ?
                AND mediafile.media_id IS NOT NULL"#,
            library_id
        )
        .fetch_all(&mut *conn)
        .await?)
    ...
    /// Returns unmatched mediafiles whose path starts with the given directory prefix.
    pub async fn get_unmatched_in_dir(
        conn: &mut crate::Transaction<'_>,
        library_id: i64,
        dir_prefix: &str,
    ) -> Result<Vec<Self>, DatabaseError> {
        ...
        Ok(sqlx::query_as!(
            MediaFile,
            "SELECT * FROM mediafile WHERE library_id = ? AND media_id IS NULL AND target_file LIKE ?",
            library_id,
            pattern
        )
        .fetch_all(&mut *conn)
