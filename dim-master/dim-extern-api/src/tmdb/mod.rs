...
mod tests {
    ...
    fn make_letterkenny() -> ExternalMedia {
        ...
        ExternalMedia {
            external_id: "tmdb:65798".into(),
            title: "Letterkenny".into(),
            original_title: Some("Letterkenny".into()),
    ...
    /// Test that a German show (Dunkelstadt) is found and returns original_title.
    #[tokio::test]
    async fn tmdb_search_german_show() {
        let provider = TMDBMetadataProvider::new("38c372f5bc572c8aadde7a802638534e");
        let provider_shows: MetadataProviderOf<TvShows> = provider.tv_shows();

        let results = provider_shows
            .search("Dunkelstadt", None, None)
            .await
            .expect("search results should exist");

        assert!(!results.is_empty(), "Should find Dunkelstadt");

        let first = &results[0];
        assert_eq!(first.title, "Dunkelstadt");
        assert_eq!(first.original_title, Some("Dunkelstadt".to_string()));
    }

    /// Test that searching with language=de-DE returns German titles, enabling
    /// matching of German-titled filenames like "Alles Steht Kopf 2".
    #[tokio::test]
    async fn tmdb_search_german_language() {
        let provider = TMDBMetadataProvider::new("38c372f5bc572c8aadde7a802638534e");
        let provider_movies = provider.movies();

        // With en-US (default), "Alles Steht Kopf 2" returns "Inside Out 2"
        let results_en = provider_movies
            .search("Alles Steht Kopf 2", Some(2024), None)
            .await
            .expect("search results should exist");
        assert!(!results_en.is_empty(), "Should find results in English");
        assert_eq!(results_en[0].title, "Inside Out 2");

        // With de-DE, the same query returns "Alles steht Kopf 2"
        let results_de = provider_movies
            .search("Alles Steht Kopf 2", Some(2024), Some("de-DE"))
            .await
            .expect("search results should exist");
        assert!(!results_de.is_empty(), "Should find results in German");
        assert!(
            results_de[0].title.to_lowercase().contains("alles steht kopf"),
            "German search should return German title, got: {}",
            results_de[0].title
        );
    }
    ...
    async fn tmdb_get_details() {
        ...
        // Details responses now carry the imdb cross-reference; the static
        // fixture doesn't track it.
        media.imdb_id = None;
