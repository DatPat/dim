...
pub(super) enum CacheKey {
    /// A search result
    Search { title: String, year: Option<i32>, language: Option<String> },
    /// Genre List
...
impl CacheEviction {
    ...
    fn evict(&self) -> usize {
        // NOTE: must be a plain load — the old `fetch_min` CLAMPED the usage
        // counter down to `max_usage` as a side effect, so the subsequent
        // `fetch_sub`s during eviction drifted the accounting below reality
        // and eviction under-fired forever after.
        if self.usage.load(Ordering::Relaxed) < self.max_usage {
            return 0;
        }
