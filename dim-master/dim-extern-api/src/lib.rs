...
/// Split a namespaced external id ("tmdb:123", "imdb:tt42") into its scheme
/// and raw value. Ids without a scheme return `(None, id)` — they predate
/// namespacing and are treated as belonging to the default provider.
pub fn parse_external_id(id: &str) -> (Option<&str>, &str) {
    match id.split_once(':') {
        Some((scheme, rest)) if !scheme.is_empty() && !rest.is_empty() => (Some(scheme), rest),
        _ => (None, id),
    }
}
...
pub struct ExternalMedia {
    ...
    pub title: String,
    /// The original-language title (e.g. native title for non-English content).
    ...
    /// The IMDB id of this media object when the provider knows it.
    #[serde(default)]
...
pub trait ExternalQuery: Debug + Send + Sync {
    /// The id-namespace scheme of this provider (e.g. "tmdb"). External ids
    /// emitted by the provider are prefixed with this scheme, and routing in
    /// chained providers dispatches on it.
    fn provider_scheme(&self) -> &'static str {
        "unknown"
    }

    /// Search by title and year. This must return a Vec of `ExternalMedia` sorted by the search
    /// score. The optional `language` parameter (e.g. "de-DE") controls the language of returned
    /// titles, enabling matching of non-English filenames.
    async fn search(&self, title: &str, year: Option<i32>, language: Option<&str>) -> Result<Vec<ExternalMedia>>;
    ...
    /// Alternative titles (regional, romaji, …) for a media by external id.
    /// Providers whose search indexes titles it doesn't return inline (TMDB)
    /// implement this so the matcher can rescue similarity-gate rejections —
    /// e.g. a romaji query against a show whose canonical titles are an
    /// English localization plus a Japanese-script original. Defaults to
    /// empty for providers without this data.
