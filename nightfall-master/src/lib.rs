...
/// Information about a running stream session, returned by `get_session_info`.
#[derive(Debug, Clone, serde::Serialize)]
pub struct SessionInfo {
    /// Profile tag (e.g. "h264_nvenc", "libx264", "h264_copy")
    pub profile_tag: String,
    /// Profile type: "Transcode", "Transmux", or "HardwareTranscode"
    pub profile_type: String,
    /// Decoder being used (e.g. "VAAPI", "V4L2 M2M", "libdav1d", "Software")
    pub decoder: String,
    /// Input codec of the source file
    pub input_codec: String,
    /// Encoding speed from ffmpeg (e.g. "2.5x")
    pub speed: Option<String>,
    /// Current frame count
    pub frame: Option<String>,
    /// Encoding fps reported by ffmpeg
...
impl StateManager {
    ...
    async fn create(
        ...
        // `Session::new` pops from the END of the chain, so the last entry is
        // the profile that actually runs first.
        let first_tag = if let Some(x) = profile_chain.last() {
            x.tag()
        } else {
    ...
    async fn chunk_init_request(&mut self, id: String, chunk: u32) -> Result<String> {
        ...
        session.check_death_and_fallback()?;
    ...
    async fn chunk_request(&mut self, id: String, chunk: u32) -> Result<String> {
        ...
        // A hardware encoder can die chunks into a stream, long after the init
        // segment was served — fall back here too, not just in init requests.
        session.check_death_and_fallback()?;
    ...
    #[handler]
    async fn get_session_info(&self, id: String) -> Result<SessionInfo> {
        let session = self
            .sessions
            .get(&id)
            .ok_or(NightfallError::SessionDoesntExist)?;

        let input_codec = session.profile_ctx.input_ctx.codec.clone();
        let profile_tag = session.profile.tag();

        // Derive the decoder method from the profile context
        let decoder = if session.profile.profile_type() == profiles::ProfileType::Transmux {
            "Copy".to_string()
        } else if session.profile_ctx.force_software_decode {
            if input_codec == "av1" {
                "libdav1d".to_string()
            } else {
                "Software".to_string()
            }
        } else if profile_tag.contains("vaapi") {
            "VAAPI".to_string()
        } else if profile_tag.contains("cuda") {
            "CUDA".to_string()
        } else if profile_tag.contains("qsv") {
            "QSV".to_string()
        } else if profile_tag.contains("v4l2") {
            // V4L2 HW decode only for h264/hevc when a decoder device exists
            #[cfg(all(unix, feature = "v4l2"))]
            {
                let hw_decode_codecs = ["h264", "hevc"];
                if profiles::v4l2::v4l2m2m_decoder_exists()
                    && hw_decode_codecs.contains(&input_codec.as_str())
                {
                    "V4L2 M2M".to_string()
                } else if input_codec == "av1" {
                    "libdav1d".to_string()
                } else {
                    "Software".to_string()
                }
            }
            #[cfg(not(all(unix, feature = "v4l2")))]
            {
                "Software".to_string()
            }
        } else {
            // Software transcode profiles
            if input_codec == "av1" {
                "libdav1d".to_string()
            } else {
                "Software".to_string()
        ...
        Ok(SessionInfo {
            ...
            profile_type: format!("{:?}", session.profile.profile_type()),
            decoder,
            input_codec,
            speed: session.get_key("speed"),
            frame: session.get_key("frame"),
            fps: session.get_key("fps"),
        })
    }
