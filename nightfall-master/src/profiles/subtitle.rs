...
impl TranscodingProfile for WebvttTranscodeProfile {
    ...
    fn supports(&self, ctx: &ProfileContext) -> Result<(), NightfallError> {
        // `webvtt`/`vtt` sources still go through ffmpeg (extraction from the
        // container), and `mov_text` is the standard MP4 text-subtitle codec.
        if ["srt", "ass", "ssa", "subrip", "webvtt", "vtt", "mov_text"]
